...
func setUpRuleExemplarsEvalTest(t require.TestingT) *teststorage.TestStorage {
	return promqltest.LoadedStorage(t, `
		load 1m
			metric{label_a="1",label_b="3"} 1 # {trace_id="1234"} 2
			metric{label_a="2",label_b="4"} 10 # {trace_id="2345"} 9
	`)
}
...
func TestRuleEvalWithZeroExemplars(t *testing.T) {
	...
	for _, scenario := range ruleEvalTestScenarios {
		t.Run(scenario.name, func(t *testing.T) {
			rule := NewRecordingRule("test_rule", scenario.expr, scenario.ruleLabels)
			result, _, err := rule.EvalWithExemplars(context.TODO(), time.Second*60, ruleEvaluationTime, EngineQueryFunc(ng, storage),
...
func TestRuleEvalWithExemplars(t *testing.T) {
	...
	for _, scenario := range ruleEvalTestScenarios {
		t.Run(scenario.name, func(t *testing.T) {
			rule := NewRecordingRule("test_rule", scenario.expr, scenario.ruleLabels)
			result, ex, err := rule.EvalWithExemplars(context.TODO(), time.Second*60, ruleEvaluationTime, EngineQueryFunc(ng, storage),
				ExemplarQuerierQueryFunc(storage.ExemplarQueryable()), nil, 0)
			require.NoError(t, err)
			require.Len(t, ex, 2)
			testutil.RequireEqual(t, scenario.expected, result)
...
func BenchmarkRuleEvalWithNoExemplars(b *testing.B) {
	...
	for _, scenario := range ruleEvalTestScenarios {
		b.Run(scenario.name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				_, _, err := rule.EvalWithExemplars(context.TODO(), time.Second*60, ruleEvaluationTime, EngineQueryFunc(ng, storage),
					ExemplarQuerierQueryFunc(storage.ExemplarQueryable()), nil, 0)
				if err != nil {
